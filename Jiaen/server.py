...
def connect_serial(port: str):
    ...
    try:
        if usb_serial and usb_serial.is_open:
            usb_serial.close()
        # 9600 BAUD for HC-05. Short timeout prevents server freezing during reads.
        usb_serial = serial.Serial(port, 9600, timeout=0.1)
        return {"status": "success"}
    except Exception as e:
        return {"status": "failed", "message": str(e)}

# --- NEW ENDPOINTS FOR HARDWARE COMMANDS ---
...
def check_status():
    ...
    if usb_serial and usb_serial.is_open:
        if usb_serial.in_waiting > 0:
            try:
                msg = usb_serial.readline().decode('utf-8').strip()
                if "OBJ_DETECTED" in msg:
