...
void loop() {
  ...
  if (stringComplete) {
    if (inputString == "RESUME") {
      ...
      digitalWrite(suckMotorPin, HIGH); // Turn on vacuum
    }
    else if (inputString == "SUCK_OFF") {
      digitalWrite(suckMotorPin, LOW); // Drop object
    }
    else {
      // It's a 6-axis PWM coordinate array from Python
      ...
      // If we successfully caught all 6 values
      if (counter == 5) {
          ...
          for(int j = 0; j < 6; j++){
              ...
              // WRITE INSTANTLY
              // Python's fast 115200 baud stream handles the smooth animation.
              // The Arduino just acts as a high-speed blind executor.
